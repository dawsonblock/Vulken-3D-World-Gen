            ...
            for (int z = 0; z < 32; z++) {
                ...
                for (int face = 0; face < 6; face++) {
                    ...
                    if (neighborPos.x < 0 || neighborPos.x >= 32 ||
                        ...
                        const auto& neighbor = voxels[static_cast<size_t>(static_cast<int>(neighborPos.x))]
                                                    [static_cast<size_t>(static_cast<int>(neighborPos.y))]
                                                    [static_cast<size_t>(static_cast<int>(neighborPos.z))];
...
OptimizedMesh GreedyMesher::quadCandidatesToMesh(const std::vector<QuadCandidate>& quads, const MeshingSettings& settings) {
    ...
    for (const auto& quad : quads) {
        ...
        // Generate vertices
        for (int i = 0; i < 4; i++) {
            OptimizedVertex& vertex = quadVertices[static_cast<size_t>(i)];
        ...
        for (int i = 0; i < 4; i++) {
            auto it = vertexMap.find(quadVertices[static_cast<size_t>(i)]);
            if (it != vertexMap.end()) {
                quadIndices[static_cast<size_t>(i)] = it->second;
            } else {
                quadIndices[static_cast<size_t>(i)] = nextVertexIndex++;
                vertexMap[quadVertices[static_cast<size_t>(i)]] = quadIndices[static_cast<size_t>(i)];
                mesh.vertices.push_back(quadVertices[static_cast<size_t>(i)]);
            }
        }
        ...
        for (int dy = -radius; dy <= radius; dy++) {
            for (int dz = -radius; dz <= radius; dz++) {
                ...
                if (nx >= 0 && nx < 32 && ny >= 0 && ny < 32 && nz >= 0 && nz < 32) {
                    if (voxels[static_cast<size_t>(nx)][static_cast<size_t>(ny)][static_cast<size_t>(nz)].blockID != 0) {
                        neighbors++;
                    }
                }
            }
