...
layout(push_constant) uniform PC { vec3 uSunDir; float uTime; } pc;
float noise(vec3 p){ return texture(uNoise, p.xz*0.0008 + pc.uTime*0.005).r; }
void main(){
    ...
    for(int i=0;i<16;i++){
        ...
        float light = clamp(dot(pc.uSunDir, vec3(0,1,0))*0.5+0.5, 0.2, 1.0);
        vec3 c = vec3(1.0)*light;
        float a = dens * 0.06 * (1.0 - alpha);
        col += c * a; alpha += a; t += step;
        if(alpha>0.98) break;
    }
    outColor = vec4(pow(col, vec3(1.0/2.2)), alpha);
}
