...
struct VulkanApp {
    ...
    VkSubmitInfo si{}; si.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO; si.waitSemaphoreCount=1; si.pWaitSemaphores=&sem_image_available; si.pWaitDstStageMask=&waitStage; si.commandBufferCount=1; VkCommandBuffer cb = cmd_bufs[imgIndex]; si.pCommandBuffers=&cb; si.signalSemaphoreCount=1; si.pSignalSemaphores=&sem_render_finished; vkResetFences(device,1,&in_flight); if(vkQueueSubmit(q_graphics,1,&si,in_flight)!=VK_SUCCESS) throw std::runtime_error("queue submit failed");
    VkPresentInfoKHR pi{}; pi.sType = VK_STRUCTURE_TYPE_PRESENT_INFO_KHR; pi.waitSemaphoreCount=1; pi.pWaitSemaphores=&sem_render_finished; pi.swapchainCount=1; pi.pSwapchains=&swapchain; pi.pImageIndices=&imgIndex; VkResult pres = vkQueuePresentKHR(q_present,&pi);
...
private:
    ...
    void create_device(){ float prio=1.f; std::vector<VkDeviceQueueCreateInfo> qs; std::vector<uint32_t> unique = {qf_graphics}; if(qf_present!=qf_graphics) unique.push_back(qf_present); for(uint32_t qf: unique){ VkDeviceQueueCreateInfo q{}; q.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO; q.queueFamilyIndex=qf; q.queueCount=1; q.pQueuePriorities=&prio; qs.push_back(q);} const char* exts[] = { VK_KHR_SWAPCHAIN_EXTENSION_NAME }; VkDeviceCreateInfo dci{}; dci.sType = VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO; dci.queueCreateInfoCount=(uint32_t)qs.size(); dci.pQueueCreateInfos=qs.data(); dci.enabledExtensionCount=1; dci.ppEnabledExtensionNames=exts; if(vkCreateDevice(phys,&dci,nullptr,&device)!=VK_SUCCESS) throw std::runtime_error("vkCreateDevice failed"); g_pipelineCache = voxelvk::util::create_pipeline_cache_from_env(device);
    ...
    void create_sync(){ VkSemaphoreCreateInfo si{}; si.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO; VkFenceCreateInfo fi{}; fi.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO; fi.flags=VK_FENCE_CREATE_SIGNALED_BIT; if(vkCreateSemaphore(device,&si,nullptr,&sem_image_available)!=VK_SUCCESS) throw std::runtime_error("sem1 failed"); if(vkCreateSemaphore(device,&si,nullptr,&sem_render_finished)!=VK_SUCCESS) throw std::runtime_error("sem2 failed"); if(vkCreateFence(device,&fi,nullptr,&in_flight)!=VK_SUCCESS) throw std::runtime_error("fence failed"); }
};
