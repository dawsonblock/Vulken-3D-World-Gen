...
#if __has_include(                                                                                                     \
    <imgui.h>) && __has_include(<imgui/backends/imgui_impl_glfw.h>) && __has_include(<imgui/backends/imgui_impl_opengl3.h>)
#include <imgui.h>
...
int main() {
    ...
    voxelvk::ai::PaletteRuntime palette_rt;
    palette_rt.load_from_file("ai_palette.cfg");
    voxelvk::ai::SetRagConfigHandler(
        [&](bool enabled, int topk) { std::printf("[RAG] (demo) enable=%d top_k=%d\n", (int)enabled, topk); });
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        ImGui::Text("RAG: %s, top-k=%d", palette_rt.cfg.ai_generation.enable_rag ? "on" : "off",
                    palette_rt.cfg.ai_generation.rag_top_k);
        ImGui::End();
