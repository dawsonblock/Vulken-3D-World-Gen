...
public:
    ...
    std::vector<float> reset() {
        ...
        agentX_ = 1.0f + static_cast<float>(rand() % (GRID_SIZE - 4));
        agentY_ = 1.0f + static_cast<float>(rand() % (GRID_SIZE - 4));
        
        // Reset target to random position (avoid agent)
        do {
            targetX_ = 1.0f + static_cast<float>(rand() % (GRID_SIZE - 4));
            targetY_ = 1.0f + static_cast<float>(rand() % (GRID_SIZE - 4));
...
int main() {
    ...
    for (uint32_t episode = 0; episode < NUM_EPISODES; episode++) {
        ...
        for (int i = static_cast<int>(episodeRewards.size()) - 1; i >= 0; i--) {
            runningAdvantage = episodeRewards[i] + 0.99f * runningAdvantage;
            episodeAdvantages[i] = runningAdvantage;
        }
