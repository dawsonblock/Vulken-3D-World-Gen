...
int main() {
        ...
        for (int i = 0; i < frames / 6; i++) {
            ...
            if (i % 10 == 0) { // Print every 10 frames
                ...
                printf("  Frame %3d: Wind=%.1f m/s, Clouds=%.2f, Precip=%.1f mm/h", 
                       i, ubo.windSpeed, ubo.cloudCoverage, ubo.precipRate);
                
                if (lightning.intensity() > 0.1f) {
                    printf(" ⚡Lightning: %.2f", lightning.intensity());
                }
                printf("\n");
            }
        }
    ...
    printf("  Wind Direction: (%.2f, %.2f, %.2f)\n", 
           finalUbo.windDir.x, finalUbo.windDir.y, finalUbo.windDir.z);
    printf("  Wind Speed: %.2f m/s\n", finalUbo.windSpeed);
    printf("  Cloud Coverage: %.2f\n", finalUbo.cloudCoverage);
    printf("  Turbidity: %.2f\n", finalUbo.turbidity);
    printf("  Time of Day: %.2f\n", finalUbo.timeOfDay);
    printf("  Sun Direction: (%.2f, %.2f, %.2f)\n",
           finalUbo.sunDir.x, finalUbo.sunDir.y, finalUbo.sunDir.z);
